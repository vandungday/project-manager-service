...
import {
  ...
  VERSION_FILE_SIZE,
  VERSION_FILE_TYPE,
} from '@/common/constants/verson.constant';
...
export class VersionController {
  ...
  @UseInterceptors(
    FileInterceptor(
      VERSION_FILE_NAME,
      setUploadOptions(VERSION_FILE_PATH, VERSION_FILE_TYPE, VERSION_FILE_SIZE),
    ),
  )
  create(
    @Body() createProjectDto: CreateVersionDto,
    @UploadedFile() file: Express.Multer.File,
  ) {
    return this.versionService.create(createProjectDto, file);
  }
  ...
  update(
    @Param('id') id: string,
    @Body() updateProjectDto: UpdateVersionDto,
    @UploadedFile() file: Express.Multer.File,
  ) {
